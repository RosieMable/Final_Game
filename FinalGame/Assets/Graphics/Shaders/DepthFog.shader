Shader "Custom/DepthFog"
{
		...
		SubShader
	{
		...
		LOD 100
		Blend SrcAlpha OneMinusSrcAlpha //for transparency blending
		...
		Pass
		{
			...
			struct appdata
			{
				float4 vertex : POSITION; //gets the position of the vertexes
			};
			...
			struct v2f
			{
				...
				float4 scrPos : TEXCOORD2; //screen position to apply the effect correctly in relation to the camera position
			};
			...
			{
				v2f o;
				o.vertex = UnityObjectToClipPos(v.vertex); //translates from obj coordinates to world coordinates
